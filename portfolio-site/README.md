...
- **Firebase's SDK still loads from Google's CDN with no SRI.** This is the one remaining third-party script source, and it's Firebase's own documented way to load it. An SRI hash isn't practical here (Firebase's modular SDK is many interdependent files, and Google updates them under the same pinned version path for security patches), so this is an accepted, low-risk trust in Google's infrastructure rather than something left half-fixed. The wallet library (`ethers.js`) no longer has this question at all — see below.
- ~~No SRI on ethers.js~~ **Resolved: vendored instead of CDN-loaded.** `assets/vendor/ethers.umd.min.js` is the exact `dist/ethers.umd.min.js` from the official `ethers@6.13.4` npm package (npm verified its own integrity on download), served same-origin. There's no longer a second CDN in the trust chain for this script at all, and no SRI hash to get right or wrong — same-origin script from your own repo needs none. Verified via a headless-browser test with a mocked wallet: the connect/balance flow works and makes zero requests to any external domain. To upgrade the version later: `npm pack ethers@<version>`, extract `dist/ethers.umd.min.js`, and replace the vendored file (see the comment at the top of `assets/js/wallet.js`).
...
│   ├── vendor/
│   │   └── ethers.umd.min.js   # ethers.js, vendored from npm — not CDN-loaded, see wallet.js
│   └── data/
...
- **Firebase SDK ยังโหลดจาก CDN ของ Google โดยไม่มี SRI** — เป็นแหล่งสคริปต์บุคคลที่สามที่เหลืออยู่จุดเดียว และเป็นวิธีที่ Firebase เอกสารแนะนำให้ทำเอง ใส่ SRI ตรงนี้ไม่ค่อยเวิร์กในทางปฏิบัติ (SDK แบบ modular ของ Firebase มีหลายไฟล์ที่โยงกัน และ Google อัปเดตไฟล์พวกนี้ภายใต้ path เวอร์ชันเดิมเวลาแพตช์ความปลอดภัย) จึงถือเป็นความเสี่ยงต่ำที่ยอมรับได้ (เชื่อใจ infrastructure ของ Google) ไม่ใช่จุดที่แก้ไม่สุด — ส่วน wallet library ไม่มีคำถามนี้อีกแล้ว ดูด้านล่าง
- ~~ไม่มี SRI บน ethers.js~~ **แก้แล้ว: เปลี่ยนจากโหลด CDN เป็น vendor ไฟล์เข้ามาในโปรเจกต์แทน** `assets/vendor/ethers.umd.min.js` คือไฟล์ `dist/ethers.umd.min.js` ตัวจริงจาก npm package `ethers@6.13.4` (npm ตรวจสอบ integrity ของแพ็กเกจเองตอนดาวน์โหลด) เสิร์ฟจาก origin เดียวกับเว็บเลย ไม่มี CDN ที่สองใน trust chain อีกต่อไป และไม่ต้องมี SRI hash ให้ใส่ผิดใส่ถูกด้วย (สคริปต์ same-origin จาก repo ตัวเองไม่ต้องมี) ทดสอบผ่าน headless browser พร้อม wallet จำลองแล้ว: การเชื่อมต่อ/เช็คยอดคงเหลือทำงานถูกต้อง และไม่มี request ออกไปโดเมนภายนอกเลยแม้แต่ครั้งเดียว จะอัปเกรดเวอร์ชันทีหลังก็แค่ `npm pack ethers@<version>` แล้วแกะเอา `dist/ethers.umd.min.js` มาแทนไฟล์ที่ vendor ไว้ (ดูคอมเมนต์บนสุดของ `assets/js/wallet.js`)
